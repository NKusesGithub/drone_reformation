...
drone_reformation [README](../README_STE.md) ·
[README_EXPLAINED](README_EXPLAINED_STE.md)
...
└── drone_reformation/
    ...
    └── docs/QUICKSTART_STE.md        # this file
```
...
on `:8000`.
**Why:** [How the network works](README_EXPLAINED_STE.md#how-the-network-works).
...
**Why:** [README_EXPLAINED §11](README_EXPLAINED_STE.md#11-each-setting-explained) gives each
...
**Why:** [The dashboard](README_EXPLAINED_STE.md#the-dashboard-on-port-8006).
...
1. **`sum(old_formation)` must equal `len(ids)`.** If it does not, some slots stay empty, or
   ...
   **Why:** [README_EXPLAINED §12.1](README_EXPLAINED_STE.md#121-sumold_formation-does-not-agree-with-lendronesids-do-this-check-first).
2. **Each row must be wider than the row in front of it.** Examples are `[N]`, `[1,2]`,
   ...
   **Why:** [README_EXPLAINED §12.10](README_EXPLAINED_STE.md#1210-most-formation-shapes-become-stuck-during-the-reformation).

To change the shape, refer to
[How to change the formation](README_EXPLAINED_STE.md#how-to-change-the-formation).
[README_EXPLAINED §11](README_EXPLAINED_STE.md#configyaml-docker-1-4-and-5-read-this-file)
...
**Why:** [README_EXPLAINED §11, `.env`](README_EXPLAINED_STE.md#env-how-the-parts-connect)
gives each variable. If Docker 1 cannot get to the bridge, do a check of the port first:
[§12.7](README_EXPLAINED_STE.md#127-the-port-number-is-different-in-different-files).
...
**Why:** [What `startup_all.sh` does](README_EXPLAINED_STE.md#what-startup_allsh-does) gives
...
[README_EXPLAINED §8, What occurs, in sequence](README_EXPLAINED_STE.md#what-occurs-in-sequence)
...
**Why:** [README_EXPLAINED §12.5](README_EXPLAINED_STE.md#125-health-gives-incorrect-data-about-mission).
...
**Why:** [README_EXPLAINED §5](README_EXPLAINED_STE.md#5-docker-1-the-gateway) gives each
route of Docker 1. It includes
[why `setup_hover` is slow](README_EXPLAINED_STE.md#setup_hover-is-slow-for-a-reason) and
[how `move_to_xy_z` becomes a go-to](README_EXPLAINED_STE.md#move_to_xy_z-becomes-the-go-to-of-crazyswarm).
...
- How mission builds the formation again:
  [`_execute_reform`, the six steps](README_EXPLAINED_STE.md#_execute_reform-the-six-steps)
- What `arrived`, `timeout` and `aborted_down` mean:
  [`_move_drone_safely`](README_EXPLAINED_STE.md#_move_drone_safely-how-a-drone-moves-to-its-spot)
- Why the simulator goes through mission, and when `AUTO_DOWN_AFTER_SEC` starts to count:
  [README_EXPLAINED §9](README_EXPLAINED_STE.md#9-docker-5-the-downed-simulator)
...
- **The one-time flags of mission never reset.** `/stop` and then `/start` do not do the
  ...
  **Why:** [README_EXPLAINED §12.6](README_EXPLAINED_STE.md#126-the-one-time-flags-never-reset).
...
| Reform fails: `Formation returned too few slots` | `sum(old_formation) < len(ids)` ([why](README_EXPLAINED_STE.md#121-sumold_formation-does-not-agree-with-lendronesids-do-this-check-first)) |
| `last_move` shows `timeout` with a `blocked_count` that increases | The shape of the formation becomes stuck (§2.4, rule 2; [why](README_EXPLAINED_STE.md#1210-most-formation-shapes-become-stuck-during-the-reformation)) |
...
[README_EXPLAINED §15, Checklist for faults](README_EXPLAINED_STE.md#15-checklist-for-faults).
...
[the status model](README_EXPLAINED_STE.md#4-the-status-model-the-core-idea). The bridge
...
[formation](README_EXPLAINED_STE.md#6-docker-3-formation-the-geometry),
[Hungarian](README_EXPLAINED_STE.md#7-docker-2-hungarian-assignment-the-matching),
[mission](README_EXPLAINED_STE.md#8-docker-4-mission-the-brain).
